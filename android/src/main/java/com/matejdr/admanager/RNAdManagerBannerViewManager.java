...
public class RNAdManagerBannerViewManager extends ViewGroupManager<BannerAdView> {
    ...
    private AdSize getCustomAdSizeFromString(String adSize) {
        AdSize adSizeValue = AdSize.BANNER;
        if(adSize != null) {
            String[] parts = adSize.split("x");
            int width = Integer.parseInt(parts[0]);
            int height = Integer.parseInt(parts[1]);
            adSizeValue = new AdSize(width, height);
        } 
        return adSizeValue;   
    }
    ...
    private AdSize getAdSizeFromString(String adSize) {
        switch (adSize) {
            ...
            default:
            return getCustomAdSizeFromString(adSize);
        }
    }
